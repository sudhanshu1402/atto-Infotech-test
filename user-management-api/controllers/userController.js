...
const createUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ id: result.insertId, email, role }, process.env.JWT_SECRET, { expiresIn: '1h' });
...
const getAllUsers = async (req, res) => {
    try {
        ...
        const users = await executeQuery('SELECT id, name, email FROM users LIMIT ? OFFSET ?', [limit, offset]);
...
const getUserById = async (req, res) => {
    try {
        ...
        const user = await executeQuery('SELECT id, name, email FROM users WHERE id = ?', [userId]);
